...
#include "../Protocol_qml/protocoldeal.h"
...
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_ConsumerFromBottom_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
...
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_ConsumerFromBottom.stringdata0))
