    ...
    for (i = 0; i < 2; i++)
    {
        if (Revversion[i] != Readversion[i])
        {
            if (Revversion[i] < Readversion[i]) // 修改代码，做测试准备
            {
                ...
                VersionComFlag = false;    // 需要升级，返回true
                return;
            }
            else
            {
...
void Protocoldeal::FreshTimer(unsigned char *time)
{
    qDebug("%s ", __PRETTY_FUNCTION__);
    if (0x01 == time[0])
    {
        qDebug("%s %x", __PRETTY_FUNCTION__, time[0]);
        QString t = Year(time);
        QString d = Time(time);
        qDebug()<< "t = "<< t << "d = "<< d;
        qDebug("emit sendTimer(t, d)");
        emit sendTimerChanged(t);
        emit sendDateChanged(d);
    }
}

QString Protocoldeal::Year(unsigned char *year)
{
    qDebug("%s ", __PRETTY_FUNCTION__);
    QString qyear;
    qyear.clear();
...
QString Protocoldeal::Time(unsigned char *time)
{
    qDebug("%s ", __PRETTY_FUNCTION__);
    QString qtime;
    qtime.clear();
