...
//        ;//emit .....  声音开关
    ...
    if(s_saf <= 39)
    {
        ...
        {
            if(m_safLast >= 0)
                emit sigFloorArrive(m_safLast%10,m_safLast/10%10,m_safLast/100,-1);
            else
                emit sigFloorArrive((-m_safLast)%10,-3,0,-1);
        }
    }
...
//    以上特殊功能只有图标，没有音频，因此插入后，重复发楼层帧不会重复播报
    ...
    if(s_saf == 40)   //maintenance
    {
        emit sigShowFuncLogo(40,-1);
    }
    if(s_saf == 41)   //out of service
    {
...
//            emit sigFloorArrive((-m_safLast)%10,-3,0,m_safLast+40);
    ...
    if(s_saf == 47)   //lift attendant
    {
        emit sigShowFuncLogo(45,-1);
    }
    if(s_saf == 48)   //photo cell
    {
...
//            emit sigFloorArrive((-m_safLast)%10,-3,0,m_safLast+40);
    ...
    if(s_saf == 57)   //emergency mode
    {
        emit sigShowFuncLogo(48,-1);
    }
    if(s_saf == 58)   //password
    {
        emit sigShowFuncLogo(49,-1);
    }
    if(s_saf == 59)      //door close  message
    {
